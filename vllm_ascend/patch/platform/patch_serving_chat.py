...
async def chat_completion_stream_generator(
        ...
        try:
            ...
            if include_usage:
                logger.info("---include usage true")
                ...
                # adaptor begin: add kv_transfer_params to stream usage response when in prefill of disaggregated mode.
                is_pd_separate_prefill = await is_disaggregated_prefill(self)
                if hasattr(last_res, 'kv_transfer_params') and last_res.kv_transfer_params and is_pd_separate_prefill:
                    final_usage_chunk.kv_transfer_params = last_res.kv_transfer_params
                    if envs_ascend.PD_DECODE_SKIP_PREPROCESS:
                        final_usage_chunk.kv_transfer_params["prompt_token_ids"] = last_res.prompt_token_ids
                        final_usage_chunk.kv_transfer_params["prefilled_token"] = [last_res.outputs[0].token_ids[0]]
                        final_usage_chunk.kv_transfer_params["stop_reasons"] = [output.stop_reason for output in last_res.outputs]
                # adaptor end   
...
async def is_disaggregated_prefill(serving_chat) -> bool:
    """
    check whether is pd_seperate_p
    """
    try:
        engine_config = await serving_chat.engine_client.get_vllm_config()
        kv_config = engine_config.kv_transfer_config
        return (kv_config.is_kv_transfer_instance and
                kv_config.is_kv_producer and
                kv_config.kv_role == "kv_producer")
 
    except Exception:
        return False
 
# vllm-project/vllm/vllm/entrypoints/openai/chat_completion/serving.py
